...
uint32_t prev_err_right_slave = 0;
uint32_t prev_ticks_right_slave = 0;
uint32_t error_i_right_slave = 0;
float control_right_slave;
uint32_t given_speed_right_slave;
uint32_t current_speed_right_slave;
...
float kp_right_slave = 5.1;
float kd_right_slave = 0.0001;
float ki_right_slave = 0.000001;
...
ISR(TIMER1_OVF_vect) {
  ...
  control_right_slave = error_right_slave * kp_right_slave + error_d_right_slave * kd_right_slave + error_i_right_slave * ki_right_slave;
  control_right = control_right + control_right_slave;

  if (control_left > 1023) {
    control_left = 1023;
  }
  if (control_right > 1023) {
    control_right = 1023;
  }
  ...
  prev_err_right_slave = error_right_slave;

  prev_ticks_left = gLeftCounter;
  prev_ticks_right = gRightCounter;
  prev_ticks_right_slave = gRightCounter;
}
